    ...
    for( unsigned int arapIteration = 0 ; arapIteration < maxIterationsForArap ; ++arapIteration ) {
        ...
        for( unsigned int v = 0 ; v < mesh.V.size() ; ++v ) {
            if(verticesHandles[v] != -1) {
                ...
                // WHAT TO PUT HERE ??????? How to update the entries of b ?
                arapLinearSystem.b(equationIndex) = mesh.V[v][0];
                equationIndex++;
                arapLinearSystem.b(equationIndex) = mesh.V[v][1];
                equationIndex++;
                arapLinearSystem.b(equationIndex) = mesh.V[v][2];
                equationIndex++;
...
void draw () {
    ...
    mesh.draw();
    drawSphere(pointSelect[0] ,pointSelect[1] ,pointSelect[2], 0.2,10,10);
    drawHandles();
...
void mouseCircle (int button, int state, int x, int y) {
    if (button == GLUT_RIGHT_BUTTON && state == GLUT_UP ) {
        ...
        gluUnProject((double)x, realy, z, modelview, projection, viewport, &xi, &yi, &zi);
