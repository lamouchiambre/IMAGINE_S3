...
std::vector<Vec3> vecteurCentroide(std::vector<Vec3> position, Vec3 target) {

    std::vector<Vec3> tmpPosition;
    
    for(int i =0; i< position.size();i++){
        Vec3 translation = target-position[i];
        tmpPosition.push_back(translation);
    }
    return tmpPosition;
}

Vec3 meanPoint(std::vector<Vec3> all_sommets){
    Vec3 mean(0.0f, 0.0f, 0.0f);
    for (int i = 0; i < all_sommets.size(); i++)
    {
        mean += all_sommets[i];
    }
    mean /=all_sommets.size();
    
    return mean;
}

Mat3 multiplicationMatrix(std::vector<Vec3> P, std::vector<Vec3> Qt) {
...
void ICP( std::vector<Vec3>  &ps , std::vector<Vec3> const &nps ,
    ...
    {
        Centroide1 = meanPoint(ps);
        Centroide2 = meanPoint(qs);

        std::vector<Vec3> pslocal = vecteurCentroide(ps, Centroide2);

        //apres deplacement pour que centroid 1 soit confondu avec centroid 2
        ...
        std::vector<Vec3> qslocal = vecteurCentroide(psNearest, Centroide2);

        Mat3 S = Mat3();

        S = multiplicationMatrix(pslocal, qslocal);
...
int main (int argc, char ** argv) {
    ...
    key ('?', 0, 0);
        ...
        loadPN("pointsets/african_statue2.pn" , positions , normals);
        loadPN("pointsets/african_statue2_subsampled_extreme.pn" , positions2 , normals2);
        ...
        ICP( positions , normals , positions2 , normals2 , qsdKdTree ,  rotation , translation ,  nIterations );
